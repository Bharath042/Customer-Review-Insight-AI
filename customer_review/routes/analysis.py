...
import pandas as pd
from models import db, User, RawText, AspectSentiment, AspectCategory # <--- NEW IMPORT: AspectCategory
...
def get_aspect_sentiment_summary(user_id):
    ...
    aspect_data_raw_query = db.session.query(
        ...
        AspectCategory.name.label('aspect_category_name'),
        AspectCategory.id.label('aspect_category_id')
    ).join(RawText).outerjoin(AspectCategory, AspectSentiment.aspect_category_id == AspectCategory.id)\
    ...
    for aspect_item in aspect_data_raw_query:
        ...
        if aspect_item.aspect_category_id:
            processed_item['aspect_group_name'] = aspect_item.aspect_category_name
